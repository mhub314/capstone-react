...
interface RhythmLogProps {
  id?: string[]
}

const RhythmLog = ( props:RhythmLogProps) => {
  ...
  const onSubmit = (data: any, event: any) => {
    ...
    if (props.id && props.id.length > 0) {
        server_calls_rhythm.update(props.id[0], data)
        ...
        server_calls_rhythm.create(store.getState())
        setTimeout( () => {window.location.reload()}, 1000 )
    }
}

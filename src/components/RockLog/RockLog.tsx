...
interface RockLogProps {
  id?: string[]
}

const RockLog = ( props:RockLogProps) => {
  ...
  const onSubmit = (data: any, event: any) => {
    ...
    if (props.id && props.id.length > 0) {
        server_calls_rock.update(props.id[0], data)
        ...
        server_calls_rock.create(store.getState())
        setTimeout( () => {window.location.reload()}, 1000 )
    }
}
    ...
    <div>
      <form className="flex" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex p-1">
          <Button className="flex justify-start m-10 mr-1 bg-pink-900 p-2 rounded hover:bg-pink-700 text-white"
          >
            Submit
          </Button>
        </div>
